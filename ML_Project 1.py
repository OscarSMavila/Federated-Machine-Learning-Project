...
class AdaptiveFedAvg(FedAvg):
    def aggregate_fit(
        ...
        # # Convert aggregated parameters back to a model
        # net = Net()
        # set_parameters(net, aggregated)

        # # Apply structured pruning on the server
        # pruned_net = structured_prune_model(net, amount=0.3)

        # return ndarrays_to_parameters(get_parameters(pruned_net)), {}
...
def structured_prune_model(model, amount=0.3):
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Conv2d) or isinstance(module, torch.nn.Linear):
            # Apply pruning first if not already pruned
            if not hasattr(module, 'weight_orig'):
                #prune.l1_unstructured(module, name="weight", amount=amount)
                prune.ln_structured(module, name="weight", amount=amount, n=2, dim=0)

            
            # Now it's safe to remove pruning
...
def set_parameters(net, parameters: List[np.ndarray]):
    ...
    # Open the log file in write mode (create it if it doesn't exist)
    with open("parameter_debug.log", "w") as log_file:
        # Print current model state_dict keys and their shapes
        log_file.write("\nCurrent model state_dict keys and their shapes:\n")
        for name, param in state_dict.items():
            log_file.write(f"{name}: {param.shape}\n")

        # Manually set parameters, check dimensions and log
        param_iter = iter(parameters)
        for name, param in state_dict.items():
            try:
                # Only assign if the shape matches
                new_param = next(param_iter)
                if new_param.shape == param.shape:
                    new_state_dict[name] = torch.Tensor(new_param)
                else:
                    # Log the mismatch if shapes don't align
                    log_file.write(f"Mismatched shapes for {name}: expected {param.shape}, got {new_param.shape}\n")
            except StopIteration:
                log_file.write("Ran out of parameters to load!\n")
                break

        # Log new state_dict with loaded parameters
        log_file.write("\nNew state_dict with loaded parameters:\n")
        for name, param in new_state_dict.items():
            log_file.write(f"{name}: {param.shape}\n")

    # Finally, load the new state_dict into the model
    net.load_state_dict(new_state_dict, strict=False)  # Use strict=False to ignore shape mismatches
